...
contract TenderEscrow is Ownable, Tender {
    ...
    function lockTokens(IERC20 _token, address _withdrawer, uint256 _amount, uint256 _unlockTimestamp) external returns (uint256 _id) {
        ...
        require(_unlockTimestamp == uint raiseUntil, 'Will only unlock after bidding session ends');
    ...
    function getEscrowedAmountByWithdrawer(address _withdrawer) view external returns (uint256[] memory) {
